...
- Batch 5B migrated provider-usage chrome across the Usage settings tab, context-meter tooltip, and compact composer session-usage summary: loading/error/empty states, refresh actions, usage/remaining labels, availability statuses, reset/run-out timing, and updated-at text. Provider names, plan labels, usage-window labels, balance/detail labels, source labels, and raw provider errors remain runtime values.
