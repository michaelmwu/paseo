...
- **Paseo-created worktrees** read `.worktreeinclude` from the live source checkout before creation. Legacy entries copy paths into the new worktree; a `# @symlink` directive makes its next path a source link. Materialization finishes before `worktree.setup` runs.
