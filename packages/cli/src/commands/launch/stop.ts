...
export async function runStopCommand(
  ...
  try {
    ...
    const payload = await client.stopWorkspaceLaunch(workspaceId, launchName);
    if (payload.error || !payload.launch) {
      throw {
        code: "WORKSPACE_LAUNCH_STOP_FAILED",
    ...
    throw toWorkspaceLaunchCommandError(
      "WORKSPACE_LAUNCH_STOP_FAILED",
      "stop workspace launch",
      error,
    );
  } finally {
