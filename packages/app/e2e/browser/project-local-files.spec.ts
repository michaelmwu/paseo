...
test("previews files without values, requires explicit replacement, imports, and remembers inclusion", async ({
  ...
  const storageKeysContainingFileContents = await page.evaluate(() =>
    Object.entries(localStorage)
      .filter(([, value]) => value.includes("FIXTURE_ONLY=local-value"))
      .map(([key]) => key),
  );
  expect(storageKeysContainingFileContents).toEqual([]);
  await page
